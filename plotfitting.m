...
mu2 = 0.550e6; % [Pa] 
Angle_record = zeros(1,361); 
Pin1_record = zeros(1,361);
Pin2_record = zeros(1,361);
M_theta1_record = zeros(1,361); 
M_theta2_record = zeros(1,361); 
...
for i = 0:1:360
    ...
    Angle_record(i+1) = i;
    % original mu
    fun_beta1 = @(beta) mu1*L*beta*(a+t).*((beta*theta+L)/L-L^3./(beta*theta+L).^3);
    int11 = integral(fun_beta1, 0, b);
    
    fun_phitao1 = @(phi,tao) mu1*((a+tao).^2.*sin(phi)+b*(a+tao)).*...
        ((L+b*theta+sin(phi).*(a+tao)*theta)/L-L^3./(L+b*theta+sin(phi).*(a+tao)*theta).^3);
    ...
    % original new mu
    fun_beta2 = @(beta) mu2*L*beta*(a+t).*((beta*theta+L)/L-L^3./(beta*theta+L).^3);
...
plot(points_x, points_y, 'k+', 'linewidth', 2);
